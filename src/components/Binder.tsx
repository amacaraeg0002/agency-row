...
import { SPORTS, CardId, Theme } from "../types";
...
export function Binder() {
  ...
  const claimReward = useGameStore((state) => state.claimCollectionReward);
  const [search, setSearch] = useState("");
  const [sport, setSport] = useState("All sports");
  const [theme, setTheme] = useState<Theme>("Live Series");
    ...
    <div>
      <div className="flex justify-between items-end">
        <div>
          ...
          <h1 className="page-title">Collection Binder</h1>
        </div>
        <div className="text-right">
          <p className="text-xs text-slate-400">Collection Progress</p>
          <p className="text-2xl font-black text-lime-300">{collectedTheme.length} / {themeIds.length}</p>
        </div>
      </div>

      <div className="mt-6 flex gap-3">
        <select className="input sm:w-48" value={theme} onChange={(e) => setTheme(e.target.value as Theme)}>
          <option value="Live Series">Live Series</option>
          <option value="Champion Heroes">Champion Heroes</option>
        </select>
      ...
      {theme === "Champion Heroes" && collectedTheme.length === themeIds.length && !game.inventory["hero-brady"] && (
         <button onClick={claimReward} className="primary-button mt-6 w-full justify-center py-4 bg-fuchsia-500 text-white">CLAIM 100 OVR TOM BRADY REWARD</button>
      )}
