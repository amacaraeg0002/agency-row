...
export const SAVE_KEY = "agency_tycoon_save_v2";
...
export function initialGame(): GameSave {
  return {
    cash: 1_000_000, // Starts with $1,000,000
    totalViews: 0,
    agencyXp: 0,
    agencyLevel: 1,
    inventory: {
      "mlb-ohtani": { count: 1, views: 30_000_000, locked: true }, // Free Maxed Out Ohtani
    },
    activeCard: "mlb-ohtani",
    staff: [],
    upgrades: { sponsor: 0, floor: 0 },
    nextId: 1,
    clockMs: 0,
    pendingPack: null,
  };
...
// Universal timing length for ALL cards (easier to hit consistently)
...
export function timing(x: number): { grade: Grade; multiplier: number } {
  // Much wider windows. Flop: 1x, Good: 10x, Perfect: 100x
...
export function trendAt(clockMs: number): Trend {
  return {
    ...
    multiplier: 2.0, // Increased trend boost
    otherMultiplier: 1.0,
...
export function calculateYield(card: Card, accumulatedViews: number, needle: number | null, trend: Trend, sponsor = 1): Yield {
  ...
  // Scaled baseviews by 150% naturally inside the math
  ...
  const gross = (views / 1_000) * (card.cpm * 1.5) * (needle === null ? 1 : sponsor); // 150% more cash
...
export function assignCard(game: GameSave, cardId: CardId | null, deskId: number | null): GameSave {
  if (cardId === null) {
    ...
    return { ...game, staff: game.staff.map((s) => s.id === deskId ? { ...s, cardId: null, paused: true } : s) };
  }
  if (deskId === null) return { ...game, activeCard: cardId };
  return { ...game, staff: game.staff.map((s) => s.id === deskId ? { ...s, cardId } : s) };
}
...
export function openPack(game: GameSave, kind: PackId): GameSave {
  ...
  const cards = Array.from({ length: spec.count }, () => {
    const value = Math.random(); // True RNG replacement
    let cumulative = 0;
    let cardId = spec.odds[spec.odds.length - 1][0];
    for (const [candidate, prob] of spec.odds) {
      cumulative += prob;
      if (value < cumulative) { cardId = candidate; break; }
    }
...
export function stepSecond(game: GameSave): { game: GameSave; productions: Production[] } {
  ...
  const sponsor = 1 + next.upgrades.sponsor * 0.25;
