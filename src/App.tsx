...
import { ArrowUpRight, BriefcaseBusiness, CircleDollarSign, Eye, Layers3, LayoutDashboard, MonitorPlay, PackageOpen, Radio, Settings2, Sparkles, TrendingUp, Zap } from "lucide-react";
import { CARDS, CARD_IDS, PARALLELS, isCardId } from "./cardData";
import { canAssign, compact, money, parallelTier, sponsorCost, sponsorMultiplier, trendAt, floorCost, floorSize, levelThreshold } from "./economy";
...
const NAV = [
  ...
  { id: "vault", label: "Pack Vault", icon: PackageOpen },
  { id: "binder", label: "Binder", icon: Layers3 }
] as const;
      ...
      <div className="mt-7 grid items-start gap-7 lg:grid-cols-[300px_1fr]">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[
              ["Base views", compact(active.baseViews)],
              ["Parallel", `${PARALLELS[tier].multiplier.toFixed(2)}×`],
              ["Sponsor", `${(1 + game.upgrades.sponsor * 0.25).toFixed(2)}×`]
            ].map(([l, v]) => (
    ...
    <div className="min-h-dvh lg:pl-64">
      ...
      <header className="sticky top-0 z-20 border-b border-white/8 bg-[#090d14]/95 backdrop-blur-xl px-5 py-4 flex flex-wrap justify-between items-center gap-4">
        <div className="flex items-center gap-4 text-xs font-bold text-lime-100">
          ...
          <span className="flex items-center gap-2 text-fuchsia-300 bg-fuchsia-950 px-2 py-1 rounded">
            <Zap size={14} /> LVL {game.agencyLevel} · {compact(game.agencyXp)} / {compact(levelThreshold(game.agencyLevel))} XP
          </span>
        </div>
      ...
      {settings && (
        <Modal title="Settings" onClose={() => setSettings(false)}>
          <button className="rounded-xl bg-rose-500 px-4 py-3 text-sm font-bold text-white w-full" onClick={() => { useGameStore.getState().reset(); setSettings(false); }}>
            Reset Game
          </button>
        </Modal>
      )}
