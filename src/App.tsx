...
import { CARDS, CARD_IDS, PARALLELS } from "./cardData";
import { compact, money, parallelTier, sponsorCost, floorCost, floorSize, levelThreshold, trendAt } from "./economy";
...
function Studio() {
  ...
  const active = CARDS[game.activeCard]; const owned = game.inventory[game.activeCard]!;
      ...
      <div className="mt-7 grid items-start gap-7 lg:grid-cols-[300px_1fr]">
        <div>
          <label className="mb-4 block text-xs text-slate-400">Active card
            <select value={game.activeCard} onChange={(e) => useGameStore.getState().assign(e.target.value, null)} className="input mt-2">
        ...
        <div className="space-y-5">
          ...
          <div className="grid grid-cols-3 gap-3">
            {[["Base views", compact(active.baseViews)], ["Parallel", `${PARALLELS[tier].multiplier.toFixed(2)}×`], ["Sponsor", `${(1 + game.upgrades.sponsor * 0.25).toFixed(2)}×`]].map(([l, v]) => (
...
function AgencyFloor({ navigate }: { navigate: (tab: Tab) => void }) {
  const game = useGameStore((state) => state.game); const events = useGameStore((state) => state.events);
  const sponsorLevel = game.upgrades.sponsor; const flvl = game.upgrades.floor;
  return (
...
export default function App() {
  const { ready } = useSession(); const [tab, setTab] = useState<Tab>("floor"); const [settings, setSettings] = useState(false);
  const game = useGameStore((state) => state.game); const message = useGameStore((state) => state.message);
  ...
  return (
    <div className="min-h-dvh lg:pl-64">
      ...
      <header className="sticky top-0 z-20 border-b border-white/8 bg-[#090d14]/95 backdrop-blur-xl px-5 py-4 flex flex-wrap justify-between items-center gap-4">
        ...
        <div className="flex gap-4"><div className="flex items-center gap-2"><Eye size={17} className="text-slate-500" /><span className="text-sm font-bold tabular-nums">{compact(game.totalViews)}</span></div><div className="flex items-center gap-2"><CircleDollarSign size={18} className="text-lime-300" /><span className="text-sm font-black tabular-nums">{money(game.cash)}</span></div></div>
      </header>

      <main className="mx-auto max-w-[1500px] px-5 py-7">
        {tab === "floor" && <AgencyFloor navigate={setTab} />} {tab === "studio" && <Studio />} {tab === "vault" && <PackOpening />} {tab === "binder" && <Binder />}
