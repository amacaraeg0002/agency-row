...
interface GameStore {
  ...
  hire: (role: StaffRole) => void; pause: (desk: number) => void; upgradeSponsor: () => void; upgradeFloor: () => void; reset: () => void; claimCollectionReward: () => void;
}
...
export const useGameStore = create<GameStore>()(persist((set, get) => ({
  ...
  buyPack: (kind) => { const game = openPack(get().game, kind); if (game === get().game) { set({ message: "Not enough cash." }); return; } set({ game, message: "Pack secured. Click cards to reveal." }); },
  reveal: (index) => {
    const game = get().game; const pending = game.pendingPack; if (!pending) return null;
    const pulled = pending.cards[index]; if (pulled.revealed) return null; const owned = game.inventory[pulled.cardId];
    const updatedCards = pending.cards.map((c, i) => i === index ? { ...c, revealed: true, duplicate: Boolean(owned) } : c);
    set({ game: { ...game, inventory: { ...game.inventory, [pulled.cardId]: { count: (owned?.count ?? 0) + 1, views: owned?.views ?? 0, locked: owned?.locked ?? false } }, pendingPack: { ...pending, cards: updatedCards } }, message: `${CARDS[pulled.cardId].name} added.` });
    return pulled.cardId;
  },
